...
namespace bx
{
	...
	{
		check(!m_running, "Already running!");
...
#	pragma pack(pop)
	...
	TlsData::TlsData()
	{
		...
		check(0 == result, "pthread_key_create failed %d.", result); BX_UNUSED(result);
	}
	...
	TlsData::~TlsData()
	{
		...
		check(0 == result, "pthread_key_delete failed %d.", result); BX_UNUSED(result);
	}
	...
	void TlsData::set(void* _ptr)
	{
		...
		check(0 == result, "pthread_setspecific failed %d.", result); BX_UNUSED(result);
	}
#endif // BX_PLATFORM_*
