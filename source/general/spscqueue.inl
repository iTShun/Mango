...
namespace bx
{
	...
	inline SpScUnboundedQueue::SpScUnboundedQueue(t_allocator* _allocator)
		: m_allocator(_allocator)
		, m_first((Node*)BX_ALLOC(m_allocator, sizeof(Node)) )
		, m_divider(m_first)
		, m_last(m_first)
	{
	}
	...
	inline SpScUnboundedQueue::~SpScUnboundedQueue()
	{
		while (NULL != m_first)
		{
			Node* node = m_first;
			m_first = node->m_next;
			BX_FREE(m_allocator, node);
		}
	}

	inline void SpScUnboundedQueue::push(void* _ptr)
	{
		m_last->m_next = (Node*)BX_ALLOC(m_allocator, sizeof(Node));
		m_last->m_next->m_ptr = _ptr;
	...
	inline SpScUnboundedQueueT<Ty>::SpScUnboundedQueueT(t_allocator* _allocator)
		: m_queue(_allocator)
	{
	}
	...
	inline SpScBlockingUnboundedQueue::SpScBlockingUnboundedQueue(t_allocator* _allocator)
		: m_queue(_allocator)
	{
	}
	...
	template<typename Ty>
	inline SpScBlockingUnboundedQueueT<Ty>::SpScBlockingUnboundedQueueT(t_allocator* _allocator)
		: m_queue(_allocator)
	{
	}
